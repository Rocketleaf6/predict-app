...
def _build_cv_link(supabase, name: str, cv_path: object) -> str:
    cv_text = str(cv_path or "").strip()
    if not cv_text:
        return ""
    public_url = supabase.storage.from_("candidates-files").get_public_url(cv_text)
    name_slug = re.sub(r"[^A-Za-z0-9]+", "_", str(name or "").strip()).strip("_") or "Candidate"
    separator = "&" if "?" in public_url else "?"
    return f"{public_url}{separator}candidate={name_slug}"
...
def render() -> None:
    ...
    for role_name in role_values:
        ...
        display_df["dob"] = display_df["dob"].fillna("").astype(str).str.strip()
        display_df["name"] = display_df.apply(
            lambda row: _build_cv_link(supabase, row.get("name", ""), row.get("cv_url", "")),
            axis=1,
        )
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "name": st.column_config.LinkColumn("Name", display_text=r"candidate=([^&]+)"),
            ...
            disabled=["name", "dob", "role", "verdict"],
        )
