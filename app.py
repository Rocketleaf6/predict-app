...
from modules.single_candidate import render as render_single
from modules.compare_candidates import render as render_compare
from modules.candidate_database import render as render_candidate_database
...
def compute_trait_scores(destiny: int, birth: int, month: int) -> Dict[str, float]:
    base_traits = {
        trait: (
            DESTINY_MATRIX[destiny][trait]
            * CORE_TRAIT_WEIGHTS.get(trait, {"Birth": 0.30, "Destiny": 0.50, "Month": 0.20})["Destiny"]
            + BIRTH_MATRIX[birth][trait]
            * CORE_TRAIT_WEIGHTS.get(trait, {"Birth": 0.30, "Destiny": 0.50, "Month": 0.20})["Birth"]
            + MONTH_MATRIX[month][trait]
            * CORE_TRAIT_WEIGHTS.get(trait, {"Birth": 0.30, "Destiny": 0.50, "Month": 0.20})["Month"]
        )
        for trait in TRAITS
        if trait != "Discipline"
    }
...
def my_candidates_page() -> None:
    ...
    required_columns = {"id", "name", "role", "verdict", "stage"}
    ...
    display_df["name"] = display_df["name"].fillna("").astype(str).str.title()
    display_df["role"] = display_df["role"].fillna("").astype(str).str.title()
    display_df["verdict"] = display_df["verdict"].fillna("").astype(str).str.upper()
    display_df["stage"] = display_df["stage"].fillna("").astype(str).map(
        lambda value: stage_labels.get(str(value).strip().title(), str(value).strip())
    )

    edited_df = st.data_editor(
        display_df[["Select", "name", "role", "verdict", "stage", "Delete"]],
        use_container_width=True,
        hide_index=True,
        key="employee_candidates_editor",
        column_config={
    ...
    for idx, row in edited_df.iterrows():
        if not bool(row.get("Delete", False)):
            continue
        candidate_row = raw_df.iloc[idx]
...
def unified_app() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("Logout"):
            ...
            st.rerun()
        if "selected_nav" not in st.session_state or st.session_state["selected_nav"] not in pages:
            st.session_state["selected_nav"] = pages[0]
        if "nav" in st.session_state and st.session_state["nav"] in pages:
            st.session_state["selected_nav"] = st.session_state["nav"]
            del st.session_state["nav"]
        selection = st.selectbox("Navigation", pages, key="selected_nav")
